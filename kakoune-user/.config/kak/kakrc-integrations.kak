...
define-command -hidden set-clipboard-commands %{
    evaluate-commands %sh{
        ...
        if command -v clip.exe >/dev/null 2>&1 && command -v powershell.exe >/dev/null 2>&1; then
            ...
            copy='clip.exe'
            paste="powershell.exe -noprofile -command \"[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; Get-Clipboard\" | tr -d '\r' | sed -z '\$ s/\n\$//'"
...
# --------------------------------------
# Standard Filetype Config

# NOTE: Some of the commands used here represent external dependencies such as:
# - newv/h commands (custom kak commands)
# - lsp-* commands  (provided by kak-lsp, above)
define-command -hidden init-std-lsp-filetype-config %{
