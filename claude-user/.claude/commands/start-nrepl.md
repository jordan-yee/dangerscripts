...
First, verify that nREPL is configured in the project configuration file:

1. Check for either a `deps.edn` (Clojure CLI) or `project.clj` (Leiningen) Clojure project configuration file.
2. Check the config file for an `:nrepl` or similar alias/profile.

If neither file exists or nREPL is not configured:
- Inform the user that nREPL is not configured
- Ask if they want you to add the nREPL configuration to the config file.
