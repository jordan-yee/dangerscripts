...
interactive (it opens a buffer in a running Kakoune); the same pages are installed
on disk as plain asciidoc, readable directly, at
`<prefix>/share/kak/doc/<topic>.asciidoc`, where `<prefix>` matches the `kak` binary
